 ...
 *      -New 상태 : new Thread()
	  	 -Runnable 상태 : start()
	  	 -Running 상태 : run()메서드를 실행중.
	  	  대기상태 (sleep) 로가면 다시 Runnable로 감.
	   	-Dead상태 : run() 종료.
	  	 *스케줄러
...
class  Thread1 extends Thread{//Thread 클래스 상속
	...
	/*
	 * Runnable  => 스케줄러(프로그래머가 관여못함.) -> Running 상태
	 */
	@Override
...
public class ThreadEx1{
	public static void main(String[] args) {
		...
		t1.start(); //Runnable 상태 , 실행가능상태
		t2.start(); 
		/*
		 * start() 메서드 : New 상태의 쓰레드를 Runnable 상태로 번경.
		 *  1, 스택을 병렬화 
		 *  2, 병렬화된 스택영역에 run() 메서드 호출
		 */
		System.out.println("main스레드 종료"); //main 메서드 종료.
