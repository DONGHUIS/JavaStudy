 ...
 *  5, setDaemon(boolean) : true인 경우 데몬 스레드로 설정됨. New상태에서 설정해야함.
 *  6, interrupt() : InterruptException 예외 발생. 대기중 스레드에 예외 발생.
 *  7, setPriority(int) : 우선순위 설정 1~10사이로 설정.
 *  					  기본값:5, 최대값:10(높음), 최소값:1(낮음)
 *  		우선순위가 높은 경우 스케줄러의 선택을 높은 확률로 가능하도록 설정.
 *  8, yield() : Running상태의 스레드를 Runnable 상태로 변경
 *  9, stop() : 스레드 강제 종료. 사용하지않는것이 권고 사항.
 *  
