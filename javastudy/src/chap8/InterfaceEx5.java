package chap8;
/*
 * jdk8 이전 : 인터페이스의 멤버는 상수와 추상메서드만 가능함.
 * jdk8 이후 : default메서드 , staitc메서드 추가됨. => 구현부 존재 가능.
 * 
 * 인터페이스 내에 구현부가 존재하는 메서드
 * default 메서드 : 인스턴스 멤버.  참조변수명.메서드명 호출
 ...
 *   
 *   static 메서드는 같은 메서드가 여러개 존재해도, 인터페이스명.메서드명으로 호출되므로 상관없다.
 */
...
public class InterfaceEx5 {
	public static void main(String[] args) {
		Child c = new Child();
		c.method();
		c.method1(); //MyInterface1.method1 default 메서드. 참조변수명 . 메서드명 호출가능.
//		c.method2(); method2는 인터페이스의 static메서드. 인터페이스명 . 메서드명 호출해야함.
//		Child.method2(); 
