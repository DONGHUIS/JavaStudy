 ...
 *   -static 중첩클래스 : new 외부클래스명.중첩클래스()
 *   -local 중첩클래스 : 메서드 내부에서 선언된 클래스. 해당 메서드에서만 접근 가능.
 *   
 * 중첩클래스의 특징
 *  1,참조자료형으로 사용됨.객체화 기능. Object 클래스의 하위클래스. 다른클래스를 상속 가능.
 *    외부클래스명.중첩클래스
 *    중첩클래스별로 바이트코드 생성.
 *  2,외부클래스의 private 멤버에 접근이 가능.  
 */
class Outer1{
	//인스턴스중첩클래스 : 외부클래스가 객체화가 되어야 클래스가됨.
	...
	//staitc 중첩클래스 : 외부클래스의 클래스멤버. 객체화 필요 없음.
	...
	void method() {
		//method() 내부에서만 사용가능.
		class LocalInner{ //local 중첩클래스 : 메서드 내부에서 선언된 클래스.  extends Object 이것이 생략되있다.
			int iv = 600;
			static int cv = 700;
...
//		=> LocalInner는 method()해당메서드 내부에서만 가능하다.
	}
